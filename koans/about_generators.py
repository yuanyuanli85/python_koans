...
class AboutGenerators(Koan):
    ...
    def test_generator_expressions_are_a_one_shot_deal(self):
        ...
        self.assertEqual(['Boom!']*3, attempt1)
    ...
    def test_generators_can_act_as_coroutines(self):
        ...
        # Therefore, calling send() with a non-None argument is prohibited when the generator iterator
        # has just started, and a TypeError is raised if this occurs (presumably due to a logic error of some kind).
        # Thus, before you can communicate with a coroutine you must first call next() or send(None) to
